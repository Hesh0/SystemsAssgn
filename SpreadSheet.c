...
int main(int argc, char *argv[]) {
   ...
   // p_SpreadSheet = malloc(GRID * sizeof(Cell));
   //assert(p_SpreadSheet);
   // p_SpreadSheet = spreadsheet;
   char resp[3];
   if (!isFirstRun())
   {
      printf("Do you want to load last spreadsheet? y/n\n");
      fgets(resp, 3, stdin);
      if (toupper(resp[0]) == 'Y')
      {
         if (loadLastSpreadSheet(spreadsheet) != true)
            printf("Could not load last SpreadSheet\n");
         else
         {
            printf("Last spreadSheet loaded\n");
   ...
   run(response, spreadsheet, value, addressPtr, formula, formulaTypePtr); //runs program until user quits
   saveLastSpreadSheet(spreadsheet);
   puts("Session saved");

   return 0;
}
// Checks if bin file exist, to see whether to prompt user to load prev session.
bool isFirstRun()
{
    struct stat st;
...
*/
void run(char *response, Cell spreadsheet[], char *value, size_t *address, char *formula, FormulaType *fType) {
   size_t addr;
   char val[BUFFER_LIMIT];
   char cacheAddr[CELL_ADDRESS];
   if (!recoveredSpreadSheet)
     initSpreadSheet(spreadsheet);
   displaySpreadSheet(spreadsheet);
   saveWorkSheet(spreadsheet);

   while (true) {
      if (!prompt(response, value))
         break;
...
*/
bool prompt(char *response, char *value) {
   printf("Enter row and column you would like to manipulate or enter 'exit' to exit program: ");
   ...
   {
      printf("Invalid cell. Enter cell to edit ('exit' to quit): ");
      fgets(response, 5, stdin);
      if (strcmp(trimEnd(response), "exit") == 0)
         return false;
      response[0] = toupper(response[0]);
   ...
   printf("Enter value: ");
   fgets(value, BUFFER_LIMIT, stdin);
   return true;
}
...
{
  renderSpreadSheet(spreadsheet, stdout);
}
...
void saveWorkSheet(Cell spreadsheet[])
{
  ...
  renderSpreadSheet(spreadsheet, file);
  fclose(file);
}
...
bool loadLastSpreadSheet(Cell spreadsheet[])
{
  ...
  {
     printf("Vals read = %d\n", valsRead);
     printf("Well sheet not all vals read\n");
     fclose(file);
     return false;
  }
  fclose(file);
